...
make = st.selectbox("Make", ['Toyota','Honda','Ford','BMW','Audi','Mercedes-Benz'])
fuel = st.selectbox("Fuel Type", ['Gasoline','Diesel','Hybrid','Electric'])
transmission = st.selectbox("Transmission", ['Automatic','Manual','CVT'])
body = st.selectbox("Body Type", ['SUV','Sedan','Hatchback','Pickup Truck'])
drivetrain = st.selectbox("Drivetrain", ['All-wheel Drive','Front-wheel Drive','Rear-wheel Drive'])

mileage = st.number_input("Mileage (in miles)", min_value=0, max_value=500000, value=45000)
car_age = st.number_input("Car Age (years)", min_value=0, max_value=50, value=4)
